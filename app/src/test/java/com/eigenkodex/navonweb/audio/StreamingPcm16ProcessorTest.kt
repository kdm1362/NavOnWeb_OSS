...
class StreamingPcm16ProcessorTest {
    ...
    @Test
    fun premiumStereoPassThroughIsIdentityAcrossArbitraryChunkSplits() {
        val format = Pcm16Format(sampleRateHz = 48_000, channelCount = 2)
        val frames = 731
        val input = ShortArray(frames * 2) { sampleIndex ->
            ((sampleIndex * 31) % 60_000 - 30_000).toShort()
        }

        val output = transform(
            tier = AudioStreamAccessTier.PREMIUM,
            format = format,
            chunks = splitFrames(input, channelCount = 2, frameCounts = listOf(1, 5, 64, 2, 199, 3)),
        )
    ...
    @Test
    fun freeMonoSixteenKilohertzPassThroughIsIdentityAcrossChunkSplits() {
        // Mono 16 kHz input already matches the FREE output format exactly, so the FREE
        // tier degenerates to pass-through just like PREMIUM does.
        val format = Pcm16Format(sampleRateHz = 16_000, channelCount = 1)
        val frames = 419
        val input = ShortArray(frames) { frame -> ((frame * 89) % 50_000 - 25_000).toShort() }

        val output = transform(
            tier = AudioStreamAccessTier.FREE,
            format = format,
            chunks = splitFrames(input, channelCount = 1, frameCounts = listOf(3, 1, 128, 7, 43)),
        )
    ...
    @Test
    fun passThroughFinishReturnsEmptyAndCountsEveryFrameExactlyOnce() {
        val processor = StreamingPcm16Processor(
            accessTier = AudioStreamAccessTier.PREMIUM,
            inputFormat = Pcm16Format(48_000, 2),
        )

        processor.process(ShortArray(10) { it.toShort() })
        processor.process(ShortArray(6) { (100 + it).toShort() })

        assertArrayEquals(ShortArray(0), processor.finish())
        assertEquals(8L, processor.inputFramesAccepted)
        assertEquals(8L, processor.outputFramesProduced)
    }

    @Test
    fun passThroughOutputIsNeverTheCallerArrayInstance() {
        val processor = StreamingPcm16Processor(
            accessTier = AudioStreamAccessTier.PREMIUM,
            inputFormat = Pcm16Format(48_000, 2),
        )
        ...
        // Ownership of the returned array transfers to the caller: mutating it afterwards
        // must not leak into the chunk the caller handed in.
        output[0] = 9
        assertEquals(1, input[0].toInt())
    }

    @Test
    fun resetAfterPassThroughStartsANewPassThroughStream() {
        val processor = StreamingPcm16Processor(
            accessTier = AudioStreamAccessTier.PREMIUM,
            inputFormat = Pcm16Format(16_000, 1),
        )
        val first = concatenate(processor.process(shortArrayOf(10, 20, 30)), processor.finish())

        processor.reset()
        val second = concatenate(processor.process(shortArrayOf(10, 20, 30)), processor.finish())

        assertArrayEquals(first, second)
        assertArrayEquals(shortArrayOf(10, 20, 30), second)
        assertEquals(3L, processor.inputFramesAccepted)
        assertEquals(3L, processor.outputFramesProduced)
    }

    @Test
    fun upsamplingIsIndependentOfChunkBoundaries() {
        val format = Pcm16Format(sampleRateHz = 8_000, channelCount = 1)
        val frames = 257
        val input = ShortArray(frames) { frame -> ((frame * 113) % 40_000 - 20_000).toShort() }
        ...
        val chunked = transform(
            tier = AudioStreamAccessTier.FREE,
            format = format,
            chunks = splitFrames(input, channelCount = 1, frameCounts = listOf(1, 2, 31, 1, 64, 5)),
        )

        assertArrayEquals(oneShot, chunked)
        assertEquals(ceilScale(frames, 16_000, 8_000), chunked.size)
    }
