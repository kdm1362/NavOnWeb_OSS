...
import com.eigenkodex.navonweb.openauto.AASDK_PING_ENCRYPTED
...
class AasdkProjectionSessionTest {
    ...
    @Test
    fun `runtime liveness ping uses encrypted post-authentication transport`() = runBlocking {
        val payload = AasdkOpenAutoProtocol.pingRequest(timestampMicros = 300L)
        val transport = BufferTransport(byteArrayOf())
        val cipher = IdentityCipher()
        val session = AasdkProjectionSession(transport, cipher)
        ...
        assertTrue(AASDK_PING_ENCRYPTED)
        assertEquals(1, cipher.encodeCount)
        assertEquals(1, transport.writes.size)
        assertArrayEquals(
