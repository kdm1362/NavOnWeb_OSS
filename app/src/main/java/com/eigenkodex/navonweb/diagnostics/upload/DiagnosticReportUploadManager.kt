...
internal class DiagnosticReportUploadManager(
    ...
    fun restorePendingUploads(): DiagnosticUploadRecoveryResult = synchronized(SUBMISSION_LOCK) {
        ...
        if (!statusStore.reconcileUserApprovedSubmission(
            ...
            return@synchronized DiagnosticUploadRecoveryResult(
                pendingCount = reportIds.size,
                ensuredCount = 0,
                failedCount = reportIds.size,
                workMigrationSucceeded = false,
            )
        }
        val namespaceMigration = scheduler.prepareWorkerNamespaceMigration(reportIds)
        if (namespaceMigration == WorkerNamespaceMigration.Failed) {
            return@synchronized DiagnosticUploadRecoveryResult(
                pendingCount = reportIds.size,
                ensuredCount = 0,
                failedCount = reportIds.size,
        ...
        val existingWorkPolicy = if (namespaceMigration is WorkerNamespaceMigration.Required) {
            ExistingWorkPolicy.REPLACE
        } else {
            ExistingWorkPolicy.KEEP
        }
        var ensuredCount = 0
        approvedSnapshot.reports.forEach { report ->
            val priorAttemptCount = (namespaceMigration as? WorkerNamespaceMigration.Required)
                ?.priorAttempts
                ?.get(report.id)
                ?: 0
            if (scheduler.enqueue(
                    ...
                    expiryDelayMillis = report.remainingRetentionMillis,
        ...
        val workMigrationSucceeded =
            namespaceMigration !is WorkerNamespaceMigration.Required ||
                ensuredCount == reportIds.size && scheduler.completeWorkerNamespaceMigration()
        DiagnosticUploadRecoveryResult(
            pendingCount = reportIds.size,
            ensuredCount = ensuredCount,
            failedCount = reportIds.size - ensuredCount,
            workMigrationSucceeded = workMigrationSucceeded,
        )
    }
...
internal data class DiagnosticUploadRecoveryResult(
    ...
    val failedCount: Int,
    val workMigrationSucceeded: Boolean,
)
