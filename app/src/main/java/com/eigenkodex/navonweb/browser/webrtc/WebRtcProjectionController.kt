...
import com.eigenkodex.navonweb.browser.BrowserWebRtcOffer
import com.eigenkodex.navonweb.browser.BrowserWebRtcOpenResult
import com.eigenkodex.navonweb.browser.BrowserWebRtcSessionSnapshot
import com.eigenkodex.navonweb.browser.BrowserWebRtcSessionState
import com.eigenkodex.navonweb.browser.BrowserWebRtcSessionTombstones
import com.eigenkodex.navonweb.browser.BrowserWebRtcSignaling
import com.eigenkodex.navonweb.browser.BrowserWebRtcSessionAdmission
import com.eigenkodex.navonweb.browser.BrowserRelayRequest
import com.eigenkodex.navonweb.browser.BrowserRelayResponse
import com.eigenkodex.navonweb.browser.BrowserSessionAccessMode
import com.eigenkodex.navonweb.browser.BrowserTouchPresenceEvent
import com.eigenkodex.navonweb.browser.browserWebRtcSessionAdmission
import com.eigenkodex.navonweb.browser.browserWebRtcEffectiveColorSlot
import com.eigenkodex.navonweb.browser.browserWebRtcSessionLimit
import com.eigenkodex.navonweb.browser.browserWebRtcSessionsToCloseForLimit
import com.eigenkodex.navonweb.browser.withDeviceDisplayName
...
import com.eigenkodex.navonweb.browser.webrtc.WebRtcCodecBridge.toBrowserCodec
import com.eigenkodex.navonweb.browser.webrtc.WebRtcCodecBridge.toPreference
import com.eigenkodex.navonweb.browser.webrtc.codec.AndroidMediaCodecCapabilityProbe
import com.eigenkodex.navonweb.browser.webrtc.codec.CodecSelectionWarning
import com.eigenkodex.navonweb.browser.webrtc.codec.EncoderAcceleration
import com.eigenkodex.navonweb.browser.webrtc.codec.MediaCodecCapabilitySnapshot
import com.eigenkodex.navonweb.browser.webrtc.codec.RemoteVideoCodecCapabilities
import com.eigenkodex.navonweb.browser.webrtc.codec.WebRtcCodecSelectionPolicy
import com.eigenkodex.navonweb.browser.webrtc.codec.WebRtcCodecSelectionResult
...
class WebRtcProjectionController(
    ...
    private val worker: ExecutorService = Executors.newSingleThreadExecutor { runnable ->
        Thread(runnable, "NavOnWebWebRtcController").apply { isDaemon = true }
    }
    private val statsScheduler = Executors.newSingleThreadScheduledExecutor { runnable ->
        Thread(runnable, "NavOnWebWebRtcStats").apply { isDaemon = true }
    }
        ...
        synchronized(lifecycleLock) {
            ...
            try {
                val defaultEncoderFactory = DefaultVideoEncoderFactory(
                    ...
                    // High stays ADVERTISED where the SDK supports it: a peer
                    // that negotiates 640c1f gets genuine High (the SDK sets
                    // KEY_PROFILE correctly on that branch). Only sessions
                    // that negotiated BASELINE are profile-enforced, so a
                    // baseline-only receiver never fixes what other peers get.
    ...
    override fun openSession(offer: BrowserWebRtcOffer): BrowserWebRtcOpenResult {
        ...
        val publicMetadata = com.eigenkodex.navonweb.browser.BrowserWebRtcSessionPublicMetadata(
    ...
    override fun reconcileSessionLimit(
        ...
        val boundedLimit = maximumSessions.coerceIn(
            com.eigenkodex.navonweb.browser.MAX_FREE_BROWSER_SESSIONS,
            com.eigenkodex.navonweb.browser.MAX_PREMIUM_BROWSER_SESSIONS,
        )
    ...
    private fun currentSessionLimit(): Int = runCatching(sessionLimitProvider)
        .getOrDefault(com.eigenkodex.navonweb.browser.MAX_FREE_BROWSER_SESSIONS)
        .coerceIn(
            com.eigenkodex.navonweb.browser.MAX_FREE_BROWSER_SESSIONS,
            com.eigenkodex.navonweb.browser.MAX_PREMIUM_BROWSER_SESSIONS,
        )
    ...
    private inner class SessionPeerObserver(
        ...
        override fun onSelectedCandidatePairChanged(event: CandidatePairChangeEvent) {
            ...
            // The adapter type is logged above and nothing is refused on it. See
            // WebRtcLocalNetworkPolicy: with the network monitor disabled this value comes from an
            // interface-name table, which is not dependable enough to end a working session over.
        }
    ...
    private companion object {
        ...
        const val VIDEO_TRACK_ID = "navonweb-projection-video"
        const val MEDIA_STREAM_ID = "navonweb-projection"
...
/**
 * Lets ICE see every interface the device actually has.
 *
 * This deliberately applies no adapter filtering. An earlier version excluded carrier and VPN
 * adapters with networkIgnoreMask and additionally killed any session whose selected pair reported
 * a carrier adapter. Both are gone, for two reasons.
 *
 * The filtering never had a real requirement behind it - it was read into a use-case description
 * rather than asked for - and ICE already prefers a local pair on its own: a host candidate on the
 * same LAN wins on priority and RTT against anything that would have to leave the device.
 *
 * More importantly the filtering became unsound once the network monitor was disabled below.
 * On the getifaddrs path libwebrtc derives adapter type from an interface-NAME table
 * (rtc::GetAdapterTypeFromName), not from ConnectivityManager. A carrier interface whose name is
 * not in that table types UNKNOWN and would have slipped through the mask anyway, while a
 * perfectly good local interface with an unusual name could type as something the old code
 * refused - and refusing meant tearing the whole session down, so a naming quirk on some
 * unfamiliar device would have looked to the user like "it just doesn't connect". A guard that
 * cannot reliably identify what it is guarding against, and whose failure mode is losing the
 * feature entirely, is worse than no guard.
 */
internal object WebRtcLocalNetworkPolicy {
    fun peerConnectionFactoryOptions(): PeerConnectionFactory.Options =
        PeerConnectionFactory.Options().apply {
            // Required for hotspot topology, which is the primary in-vehicle setup.
            //
            // libwebrtc's default Android network enumerator is driven by
            // ConnectivityManager, and ConnectivityManager does not report the
            // phone's own softAP interface as a network. When the phone IS the
            // access point, the enumerator therefore sees nothing but loopback:
            // every answer we generate carries only 127.0.0.1/::1 candidates, the
            // sockets are loopback-bound, and no direct-LAN session can form in
            // either direction - even though the softAP interface holds the very
            // address the HTTP server is answering on, the generated answers
            // advertise loopback only and every kind of client fails identically.
            //
            // Android always enumerates with getifaddrs; the monitor does not
            // enumerate, it gates. With it on, any non-loopback interface missing
            // from the ConnectivityManager-derived map is marked unavailable and
            // dropped, while loopback bypasses that gate - which is exactly why
            // the failure produced loopback-only candidates rather than none.
            //
            // Cost: connectivity changes are no longer pushed from Android, so
            // regathering reacts on ICE's own timers rather than immediately.
...
private class MicrophoneFrameDispatcher(
    ...
    private companion object {
        ...
        const val THREAD_NAME = "NavOnWebWebRtcMicrophone"
    }
}
