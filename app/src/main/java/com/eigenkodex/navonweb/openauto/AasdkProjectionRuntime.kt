...
import com.eigenkodex.navonweb.openauto.protocol.AasdkAudioChannelController
import com.eigenkodex.navonweb.openauto.protocol.AasdkMicrophoneChannelController
import com.eigenkodex.navonweb.openauto.protocol.AasdkMicrophoneOpenStatus
import com.eigenkodex.navonweb.openauto.protocol.AasdkOpenAutoProtocol
import com.eigenkodex.navonweb.openauto.protocol.AasdkProjectionSession
import com.eigenkodex.navonweb.openauto.protocol.AasdkProjectionSessionConnector
import com.eigenkodex.navonweb.openauto.protocol.AasdkProtocolException
import com.eigenkodex.navonweb.openauto.protocol.AasdkSessionMessage
import com.eigenkodex.navonweb.openauto.protocol.AasdkTcpTransport
...
import com.eigenkodex.navonweb.openauto.protocol.AasdkVideoPacketCodec
import com.eigenkodex.navonweb.openauto.protocol.AasdkVoiceSessionStatus
import com.eigenkodex.navonweb.openauto.sensor.FailDarkNightModeStateProvider
import com.eigenkodex.navonweb.openauto.sensor.NightModeDecision
import com.eigenkodex.navonweb.openauto.sensor.NightModeStateProvider
...
internal const val AASDK_PING_ENCRYPTED = true
...
internal class AasdkProjectionRuntime(
        ...
        val pinger = launch {
            while (true) {
                ...
                session.sendMessage(
                    ...
                    encrypted = AASDK_PING_ENCRYPTED,
                )
            }
        }
            ...
            launch {
                ...
                while (true) {
                    ...
                    if (transmission == null) {
                        // Two cadences: while streaming, an empty result means
                        // the send window is full or the source is briefly dry,
                        // and the short backoff keeps frame latency low. While
                        // idle, nextTransmission returns null immediately, so
                        // the short backoff would wake this loop 200 times per
                        // second for nothing; the slow cadence delays only the
                        // first frame after activation, by at most one idle
                        // poll interval, well inside voice session setup time.
                        delay(
                            if (controller.isStreaming()) {
                                MICROPHONE_EMPTY_POLL_BACKOFF_MILLIS
                            } else {
                                MICROPHONE_IDLE_POLL_MILLIS
                            },
                        )
                        continue
                    }
    ...
    private companion object {
        ...
        const val MICROPHONE_IDLE_POLL_MILLIS = 100L
